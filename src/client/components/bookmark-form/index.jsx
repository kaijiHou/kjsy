...
export default class BookmarkIndex2 extends PureComponent {
  ...
  renderTypes (bookmarkType, isNew, keys) {
    ...
    const featureTypes = keys.filter(k => k === connectionMap.ssh && features.ssh)
    const filtered = window.et && Array.isArray(window.et.supportSessionTypes)
      ? featureTypes.filter(k => window.et.supportSessionTypes.includes(k))
      : featureTypes
  ...
  renderAIButton (isNew) {
    if (!features.ai || !isNew || this.state.aiMode || isAIDisabled()) {
      return null
    }
    return (
      <Button
