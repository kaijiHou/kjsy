...
import { Form, Input, InputNumber, Switch, Select, AutoComplete, Alert, Radio, Collapse } from 'antd'
  ...
  switch (type) {
    ...
    case 'collapse':
      return (
        <Collapse
          key={name}
          className='easyssh-advanced-options'
          items={[{
            key: name,
            label: (
              <span>
                <b>{item.label}</b>
                {item.description && <span className='easyssh-advanced-description'>{item.description}</span>}
              </span>
            ),
            children: (
              <div className='easyssh-advanced-fields'>
                {(item.fields || []).map((field, childIndex) => (
                  renderFormItem(field, formItemLayout, form, ctxProps, `${index}-${childIndex}`)
                ))}
              </div>
            )
          }]}
