...
export default class AttachAddonCustom {
  ...
  _initBase = async () => {
    ...
    // 增强 _sendData：socket 未 OPEN（CONNECTING）时缓冲输入，OPEN 后按序重放。
    // 原版在 readyState !== OPEN 时静默丢弃，导致连接/重连期间用户输入无回显、
    // 命令丢失，表现为"终端像卡死"（实际 shell 正常）。
    this._pendingInput = []
    this._replayBound = false
    const origSend = base._sendData.bind(base)
    base._sendData = (data) => {
