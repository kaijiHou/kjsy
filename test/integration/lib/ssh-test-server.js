 ...
 *
 * The host key is generated in memory for each test process. The server binds
 * 127.0.0.1 and accepts only the test credentials, so no private key material
...
const HOST_KEY = utils.generateKeyPairSync('ed25519', {
  comment: 'electerm-integration-test'
})
const HOST_KEY_PRIVATE = HOST_KEY.private
const HOST_KEY_PUBLIC = HOST_KEY.public
...
function ensureKnownHostsEntry (port = TEST_PORT, log = () => {}) {
  ...
  const hostPrefix = `[127.0.0.1]:${port} `
  const kept = content.split(/\r?\n/).filter(line => line && !line.startsWith(hostPrefix))
  fs.mkdirSync(path.dirname(knownHostsPath), { recursive: true, mode: 0o700 })
  fs.writeFileSync(knownHostsPath, `${kept.concat(entry).join('\n')}\n`, { mode: 0o600 })
