...
const sshBookmarkSchema = {
  ...
  // === EasySSH 实验室服务器元数据（全部 optional，兼容 electerm 原版数据）===
  easysshGroup: z.string().optional().describe('EasySSH lab server group'),
  easysshGpuModel: z.string().optional().describe('EasySSH GPU model'),
  easysshGpuCount: z.number().optional().describe('EasySSH GPU count'),
  easysshDefaultRemotePath: z.string().optional().describe('EasySSH default remote path for SFTP'),
  easysshFavorite: z.boolean().optional().describe('EasySSH favorite flag'),
  easysshTags: z.array(z.string()).optional().describe('EasySSH tags'),
  easysshMonitorEnabled: z.boolean().optional().describe('EasySSH GPU monitor enabled')
}
